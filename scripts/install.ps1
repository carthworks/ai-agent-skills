...
#     .\scripts\install.ps1 -Target "plugins/fullstack-quality-pack"
#     .\scripts\install.ps1 -Target "skills/web/web-trust-and-compliance"
#     .\scripts\install.ps1 -Target "agents/code-reviewer"
#     .\scripts\install.ps1 -Target "rules/typescript-strict-guardrails.md"
...
$Catalogue = @{
    ...
    15 = @{ Path = "agents/security-auditor";                  Label = "[Agent]   security-auditor          - AppSec & vulnerability scanner" }
    16 = @{ Path = "agents/code-reviewer";                     Label = "[Agent]   code-reviewer             - Principal PR code reviewer" }
    17 = @{ Path = "agents/qa-engineer";                       Label = "[Agent]   qa-engineer               - Test plan & regression generator" }
...
function Install-Item-Path ($itemRel, $extractedRoot) {
    ...
    $Aliases = @{
        "plugins/quality-core"              = "plugins/fullstack-quality-pack"
        "plugins/fullstack-launch"          = "plugins/production-launch-pack"
        "plugins/secure-delivery"           = "plugins/web-security-pack"
        "agents/pr-reviewer"                = "agents/code-reviewer"
        "agents/pr-reviewer.agent.json"     = "agents/code-reviewer"
        "agents/qa-tester"                  = "agents/qa-engineer"
        "agents/qa-tester.agent.json"       = "agents/qa-engineer"
        "agents/security-auditor.agent.json"= "agents/security-auditor"
        "agents/code-reviewer.agent.json"   = "agents/code-reviewer"
        "agents/qa-engineer.agent.json"     = "agents/qa-engineer"
        "rules/git-safety"                  = "rules/token-efficiency.md"
        "rules/git-safety.md"               = "rules/token-efficiency.md"
        "rules/typescript-strict"           = "rules/typescript-strict-guardrails.md"
        "rules/typescript-strict.md"        = "rules/typescript-strict-guardrails.md"
        "rules/web-compliance"              = "rules/clean-architecture-boundaries.md"
        "rules/web-compliance.md"           = "rules/clean-architecture-boundaries.md"
        "rules/secret-hygiene"              = "rules/security-and-secret-hygiene.md"
        "rules/secret-hygiene.md"           = "rules/security-and-secret-hygiene.md"
    }
    if ($Aliases.ContainsKey($normRel)) {
        $normRel = $Aliases[$normRel]
    }
...
try {
    ...
    if ($Target -ne "") {
        if ($Target -eq "all") {
            ...
            Get-ChildItem (Join-Path $ExtractedRoot "agents") -Directory | ForEach-Object {
                Copy-Item -Recurse -Force $_.FullName (Join-Path ".agents\agents" $_.Name)
            }
